...
import br.com.waiso.framework.io.ProgressFileGeneration;
...
import br.com.waiso.framework.zip.CreateZipFile;
import br.com.waiso.framework.zip.ZipContent;
...
public abstract class GerenciadorZipView extends MonitorStatus {
	...
	public void downloadZipFile(ServletRequest request, ServletResponse response){
		...
		if (zipContents.isEmpty()){
			JSONReturn jsonReturn = JSONReturn.newInstance(Consequence.ERRO).message("Nenhum arquivo foi encontrado para geração.");
		...
		try{
			...
			for(ZipContent zipContent : zipContents){
				...
				updateStatusZip(request, ticket, JSONReturn.newInstance(Consequence.ATENCAO, progress).message("Zipando [" + x + "/" + zipContents.size() + "]: " + progress.getCurrentFileGeneration() ));
				zipFile.add(zipContent);
			}
			updateStatusZip(request, ticket, JSONReturn.newInstance(Consequence.SUCCESSO, progress).message("Arquivo pronto para download"));
			zipFile.close();
		}catch(IOException e){
			updateStatusZip(request, ticket, JSONReturn.newInstance(Consequence.ERRO, progress).message(GerenciadorMensagem.getMessage(GerenciadorMensagem.ERROR_GERAL)));
