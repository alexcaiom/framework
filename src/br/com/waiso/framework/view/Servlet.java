...
import br.com.waiso.framework.exceptions.ErroCriticoUsuario;
...
public abstract class Servlet<T> extends HttpServlet {
	...
	protected T executeWebClass(ServletRequest request, ServletResponse response, Object webClass, String invoke) {
		...
		try{
			...
			throw new ErroUsuario(message);
		}catch(InvocationTargetException e){
			e.printStackTrace();
			if(e.getTargetException() instanceof ErroUsuario){
				throw (ErroUsuario) e.getTargetException();
			}
			String message = GerenciadorMensagem.getMessage(GerenciadorMensagem.ERROR_GERAL);
			throw new ErroCriticoUsuario(Servlet.class, message, e);
