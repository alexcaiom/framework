 ...
 * excecoes de conversao  @link{com.waiso.framework.exceptions.ConverterException} 
 * e excecao de restricao quando o required eh informado, porem todas as excecoes sao lancadas como
 * @link{com.waiso.framework.exceptions.UserLinkException}. Uma forma de
...
public class InputArray<T> implements IComponentView<T> {
	...
	public static class Builder<T>{
		...
		public InputArray<T> build() {
				...
				if (isNullValue()){ //Se for nulo entao vamos ver se ele eh requerido
					...
					if (regexValidation != null){
						if (regexValidation.evaluate(value)){
							...
							ErroLinkUsuario userLinkException = new ErroLinkUsuario(name, message);
							InputHolder.get().add(userLinkException);
						}
					}
			...
			}catch(ErroConversor e){
				String message = null;
				if (e.getClass() == ErroConversor.class){
					String nameType = type.getSimpleName();
					String messageKey = "framework.utils."+ nameType.toLowerCase()+".with.invalid.field.array";
